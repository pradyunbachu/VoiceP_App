"""Daily recommendations route.

GET  /daily-recs   — Returns time-of-day-aware meal suggestions (breakfast,
     lunch, dinner, or snack) generated by Groq (Llama 3.3 70B) using the
     user's current pantry inventory. Also surfaces low-stock alerts and
     items expiring within 3 days. Cached for 30 min per meal type.

POST /recipe-detail — Given a meal name from the daily recs, generates a
...
def detect_meal_type(tz: str | None = None) -> str:
    ...
    # Time boundaries: breakfast < 11, lunch < 16, dinner < 21, else snack
    if hour < 11:
        return "breakfast"
    elif hour < 16:
        return "lunch"
...
def generate_meal_recs(ingredient_list: str, expiring_list: str, meal_type: str):
    ...
    # Steer the LLM toward appropriate dishes for each meal type
    meal_hints = {
...
async def get_daily_recs(
    ...
    # If nothing is explicitly low-stock, flag a couple of quantity-1 items as a gentle restock nudge
    if not low_stock:
        qty_one_items = [
    ...
    # Surface items expiring within 3 days so the LLM can prioritize them
