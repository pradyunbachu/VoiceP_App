# VoiceP - Voice Powered Expense Tracker

A modern, AI-powered expense tracking application that allows you to record expenses using your voice. Simply speak about your purchase, and the app automatically extracts store name, items, amount, and date, then displays it in a beautiful analytics dashboard.

## 🎯 Features

- **🎤 Voice Recording**: Record expenses using your microphone with Web Speech API (free, browser-based)
- **🤖 AI-Powered Extraction**: Automatically extracts store, items, amount, and date from voice transcripts using Groq LLM
- **📊 Analytics Dashboard**: Visualize spending with interactive charts (line, bar, and pie charts)
- **💾 Expense History**: View and manage all recorded expenses with delete functionality
- **🎨 Modern Dark UI**: Sleek black theme with glassmorphism effects and neon accents
- **📱 Responsive Design**: Works seamlessly on desktop and mobile devices
- **🔄 Smart Fallbacks**: Automatically falls back to regex-based extraction if AI services are unavailable
- **✍️ Manual Entry**: Type expenses manually if voice recording isn't available
...
- **Groq LLM** - Ultra-fast AI inference engine for expense extraction
  - Uses `llama-3.1-70b-versatile` model for accurate JSON extraction
...
- **SQLite** - Lightweight, file-based database
  ...
  - Tracks: store, items, amount, date, and timestamps
  - Provides analytics aggregation (totals, by store, by date)
...
- **Recharts** - Composable charting library built on React
  ...
  - Pie charts for expense distribution by store
...
3. **Expense Extraction**: Transcript sent to backend → Groq LLM extracts structured data
...
- `.env` - Contains API keys (Groq, OpenAI if used)
...
- `GET /api/analytics` - Get analytics data (totals, by store, by date)
- `DELETE /api/expenses/{id}` - Delete an expense
- `GET /api/db-viewer` - View database contents in browser (development tool)

## 🎨 UI/UX Features

- **Dark Theme**: Modern black background with neon accent colors
- **Glassmorphism**: Translucent cards with backdrop blur effects
- **Smooth Animations**: Hover effects, transitions, and loading states
- **Responsive Design**: Works on desktop, tablet, and mobile
- **Navigation**: Fixed top bar with tab-based navigation
- **Landing Page**: Beautiful introduction screen with animated logo
