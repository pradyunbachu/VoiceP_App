...
const SpendingInsights: React.FC<Props> = ({ showToast }) => {
  ...
  const getChangeIcon = (change: number | undefined): React.ReactNode => {
  ...
  const getChangeColor = (change: number | undefined, invertColors: boolean = false): string => {
  ...
  const getFindingIcon = (type: KeyFinding['type']): React.ReactNode => {
    switch (type) {
      case 'positive':
  ...
  const getPriorityBadge = (priority: Recommendation['priority']): React.ReactNode => {
    return (
        ...
        <div className="summary-card">
          ...
          <div className="card-content">
            ...
            <span className="card-value">
              {budget_status ? (
                budget_status.filter((b: BudgetStatus) => b.status === 'ok').length + '/' + budget_status.length
              ) : (
                'N/A'
              )}
            </span>
          ...
          {ai_insights.key_findings && ai_insights.key_findings.length > 0 && (
            <div className="findings-section">
              ...
              <div className="findings-list">
                {ai_insights.key_findings.map((finding: KeyFinding, index: number) => (
          ...
          {ai_insights.recommendations && ai_insights.recommendations.length > 0 && (
            <div className="recommendations-section">
              ...
              <div className="recommendations-list">
                {ai_insights.recommendations.map((rec: Recommendation, index: number) => (
      ...
      <div className="breakdown-section">
        ...
        {top_categories && top_categories.length > 0 ? (
          <div className="category-bars">
            {top_categories.map((cat: TopCategory, index: number) => (
      ...
      <div className="breakdown-section">
        ...
        {top_stores && top_stores.length > 0 ? (
          <div className="stores-grid">
            {top_stores.map((store: TopStore, index: number) => (
      ...
      {budget_status && budget_status.length > 0 && (
        <div className="breakdown-section">
          ...
          <div className="budget-grid">
            {budget_status.map((budget: BudgetStatus, index: number) => (
