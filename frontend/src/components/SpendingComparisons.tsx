...
interface ComparisonHighlight extends CategoryComparison {
  difference?: number;
}
...
const SpendingComparisons: React.FC<Props> = ({ showToast }) => {
  const now = new Date();
  ...
  const [compareMonth, setCompareMonth] = useState<number>(defaultCompareMonth);
  ...
  const { summary, category_comparisons, store_comparisons, sentences, biggest_increase, biggest_decrease, current_period, compare_period } = comparison as SpendingComparisonType & {
    biggest_increase: ComparisonHighlight | null;
    biggest_decrease: ComparisonHighlight | null;
  };

  // Calculate max amount for bar scaling
  const maxCategoryAmount = Math.max(
    ...category_comparisons.map((c: CategoryComparison) => Math.max(c.current_amount, c.previous_amount)),
    1
  );

  const maxStoreAmount = Math.max(
    ...store_comparisons.map((s: StoreComparison) => Math.max(s.current_amount, s.previous_amount)),
    1
  );
        ...
        <div className="comparisons-controls">
          <div className="month-selector">
            <select
              ...
              value={currentMonth}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCurrentMonth(Number(e.target.value))}
            >
              {MONTHS.map((m) => (
            ...
            <select
              className="month-select"
              value={currentYear}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCurrentYear(Number(e.target.value))}
            >
            ...
            <select
              className="month-select"
              value={compareMonth}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCompareMonth(Number(e.target.value))}
            >
              {MONTHS.map((m) => (
                <option key={m.value} value={m.value}>{m.label}</option>
              ))}
            ...
            <select
              className="month-select"
              value={compareYear}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCompareYear(Number(e.target.value))}
            >
              {getYearOptions().map((y) => (
                <option key={y} value={y}>{y}</option>
              ))}
          ...
          {biggest_increase && (
            <div className="highlight-card increase">
              ...
              <div className="highlight-detail">
                ...
                {' '}(+${(biggest_increase as ComparisonHighlight).difference?.toFixed(2)})
              </div>
            </div>
          )}
          {biggest_decrease && (
            <div className="highlight-card decrease">
              ...
              <div className="highlight-detail">
                ...
                {' '}(-${Math.abs((biggest_decrease as ComparisonHighlight).difference ?? 0).toFixed(2)})
              </div>
            </div>
          )}
        </div>
      ...
      <div className="comparison-bars-section">
        ...
        {category_comparisons && category_comparisons.length > 0 ? (
          <div className="comparison-bar-list">
            {category_comparisons.slice(0, 8).map((cat: CategoryComparison, index: number) => (
      ...
      <div className="comparison-bars-section">
        ...
        {store_comparisons && store_comparisons.length > 0 ? (
          <div className="comparison-stores-grid">
            {store_comparisons.slice(0, 8).map((store: StoreComparison, index: number) => (
