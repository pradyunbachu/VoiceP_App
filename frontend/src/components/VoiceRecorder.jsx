  ...
  return (
    <div className="voice-recorder">
      ...
      <ExpenseResult
        ...
        onSetPendingExpense={(expense) => { processor.setPendingPantryExpense(expense); setShowPantryModal(true); }}
      />
      ...
      {showReceiptScanner && (
        <ReceiptScanner
          ...
          onSuccess={(result) => {
            processor.setExtractedExpense({ expenses: [result], count: 1, message: result.message });
            const category = (result.category || "").toLowerCase();
            const foodCategories = ["groceries", "grocery", "dining", "restaurant", "food"];
            if (foodCategories.some(cat => category.includes(cat))) {
              processor.setPendingPantryExpense({ ...result, id: result.expense_id });
            }
